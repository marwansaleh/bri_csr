...
switch($subject){
    ...
    case 'editPropinsi': getProvinces($db_obj); break;
    case 'unikPropinsi': uniqueProvinces($db_obj); break;
    default: error_404();
}
...
function getProvinces(Database $db_obj){
    $id = isset($_GET['id']) ? $_GET['id'] : NULL;
    if($id){
        $result = _get_provinces($id,$db_obj);
    }
    _send_output($result);
}

function _get_provinces($id, Database $db_obj){
    if (!$db_obj) $db_obj = new Database();
    $sql = 'SELECT * FROM propinsi where id ='. $id;    
    return $db_obj->get_single_row($sql);
}

function uniqueProvinces(Database $db_obj){
    $id = isset($_GET['id']) ? $_GET['id'] : NULL;
    $nama = isset($_GET['propinsi']) ? $_GET['propinsi'] : NULL;
    if($id && $nama){
        $result = _unique_provinces($id,$nama,$db_obj);
    }
    _send_output($result);
}

function _unique_provinces($id,$nama, Database $db_obj){
    if (!$db_obj) $db_obj = new Database();
    $sql = 'SELECT * FROM propinsi where propinsi = "'.$nama.'" and id !='. $id;    
    $result = count($db_obj->get_single_row($sql));
    return $result?'1':'0';
}
