...
<script type="text/javascript"> 
    ...
    $(document).ready(function(){
        loadPrograms(0,$('select#propinsi').val(),'');
        ...
        $('select#propinsi').change(function(){
            ...
            loadPrograms(0, propinsi, keyword);
        })
        $('div#btn_search_content').click ( function ()
	{
            var keyword = $(this).parent().find('input').val();
            var propinsi = $('select#propinsi').val();
            loadPrograms(0, propinsi, keyword);
	});
    ...
    function loadPrograms(page,propinsi,keyword)
    {        
        $('div#my-loader').show();
        $.post("ajax",{input_function:'loadProgramsByPropinsi',param:page,propinsi:propinsi,search_str:keyword},function(result){
            $('div#my-loader').hide();
            ...
            if (data['found']>0){
                ...
                createNavigator(page, propinsi, keyword, data['pages']);
            }else{
    ...
    function createNavigator(page_active, propinsi, keyword, num_of_pages)
    {
        //clear old navigation
        $('ul.navigation').empty();
        ...
        if (num_of_pages>1){
            for(var i=0;i<num_of_pages;i++){
                var s="<li onclick='loadPrograms("+i+",\""+propinsi+"\",\""+keyword+"\");'";
...
<body>
    ...
    <div id="panel-content">
        <div class="content">
            <h1>Program CSR BRI - Berdasarkan Propinsi</h1>
